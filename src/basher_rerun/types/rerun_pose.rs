...
impl RerunQuadPose {
    ...
    pub fn log_vector(
        &self,
        prefix: &str,
        stream: &mut RecordingStream,
        name: &str,
        vec: Vector3<f64>,
    ) {
        let point = Points3D::new([Vec3D::new(vec.x as f32, vec.y as f32, vec.z as f32)])
            .with_labels(vec![prefix]);
    ...
    pub fn log_pose(&self, prefix: &str, stream: &mut RecordingStream) {
        ...
        self.log_vector(
            ...
            self.pose.angular_acceleration,
        );
    }
}
